...
interface FefoEstoqueXPickingTabProps {
...
export const FefoEstoqueXPickingTab: React.FC<FefoEstoqueXPickingTabProps> = ({
  validadesList,
  user,
  empresa,
  onRefresh
  ...
  // Filter only Estoque x Picking items
  const estoqueXPickingItems = useMemo(() => {
    return historyItems.filter(item => item.tipo === 'ESTOQUE_X_PICKING');
  }, [historyItems]);

  // Apply all filters
  const filteredItems = useMemo(() => {
    return estoqueXPickingItems.filter(item => {
      // Month filter
      ...
      if (searchTerm.trim()) {
        ...
        if (!mCod && !mDesc && !mLoteV && !mLoteN && !mPosO && !mResp) return false;
      }
  ...
  }, [estoqueXPickingItems, selectedMes, statusFilter, gravidadeFilter, ruaFilter, searchTerm, useCustomDateRange, startDate, endDate, dateField]);
  ...
  const handleSaveEdit = () => {
    ...
    showFeedback('Tratativa da Quebra Estoque x Picking salva com sucesso!');
  };
  ...
  const handleExportExcel = () => {
    const rows = filteredItems.map((item, idx) => ({
      ...
      'Tipo de Quebra': 'Estoque x Picking',
      'Pulmão Origem': item.posicaoOrigem,
      'Validade Pulmão': item.validadeMaisVelho,
      'Destino Picking': item.posicaoDestino,
      'Validade Picking': item.validadeMaisNovo,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Quebras_Estoque_x_Picking');
    XLSX.writeFile(wb, `Quebras_Estoque_x_Picking_${companyId}_2026.xlsx`);
  ...
  const handleCompleteAllVisible = () => {
    ...
    if (window.confirm(`Deseja marcar todas as ${filteredItems.length} quebras visíveis de Estoque x Picking como CONCLUÍDAS?`)) {
      ...
      showFeedback(`${filteredItems.length} quebras de Estoque x Picking marcadas como Concluídas!`);
    }
  };
        ...
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="bg-blue-800 text-blue-100 text-[10px] font-bold px-2 py-0.5 rounded-md border border-blue-700">
              Pulmão de Estoque x Área de Picking
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black tracking-tight flex items-center gap-2.5">
            ...
            <span>Auditoria &amp; Histórico de Quebras Estoque x Picking</span>
          </h2>
          <p className="text-xs text-blue-100/90 mt-1 max-w-3xl leading-relaxed">
            Monitoramento de lotes mais antigos esquecidos nas posições de pulmão enquanto o picking é abastecido com produtos mais novos.
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
          ...
          <div className="mt-2">
            ...
            <span className="text-[10px] text-slate-500 block font-bold mt-0.5">Inversões de Picking</span>
          </div>
        </div>
        ...
        <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200 shadow-xs flex flex-col justify-between">
          ...
          <div className="mt-2">
            ...
            <span className="text-[10px] text-amber-800 block font-bold mt-0.5">Aguardando abastecimento</span>
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl">
            ...
            >
              Todos ({estoqueXPickingItems.length})
            </button>
            <button
              type="button"
            ...
            >
              Pendente ({estoqueXPickingItems.filter(i => i.status === 'pendente').length})
            </button>
            <button
              type="button"
            ...
            >
              Em Andamento ({estoqueXPickingItems.filter(i => i.status === 'em_andamento').length})
            </button>
            <button
              type="button"
            ...
            >
              Concluído ({estoqueXPickingItems.filter(i => i.status === 'concluido').length})
            </button>
          </div>
          ...
          <div className="relative flex-1 min-w-[220px] max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar SKU, Lote, Descrição, Pulmão..."
              value={searchTerm}
        ...
        {/* Linha 2: Filtros de Gravidade & Pulmão */}
        <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 text-slate-800 text-[11px] font-bold rounded-lg px-2.5 py-1.5 focus:outline-none"
            >
              <option value="todos">Todos os Pulmões</option>
              {availableRuas.map(r => (
                <option key={r} value={r}>Rua Pulmão {r}</option>
              ))}
            </select>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <h3 className="font-sans font-black text-xs uppercase text-[#032b5e] tracking-wider mt-2">
              Inversões Pulmão vs Picking (Dias)
            </h3>
          </div>
          ...
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-[10px] text-slate-500 font-medium">
            💡 O lote mais antigo retido no pulmão deve ser abastecido no picking imediatamente.
          </div>
        </div>

        {/* Gráfico 2: Quebras por Pulmão */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <h3 className="font-sans font-black text-xs uppercase text-[#032b5e] tracking-wider mt-2">
              Ruas de Pulmão com Maior Retenção
            </h3>
          </div>
          ...
          <div className="bg-sky-50 p-2.5 rounded-xl border border-sky-100 text-[10px] text-sky-800 font-bold">
            🚜 Recomendação: Puxar paletes de reposição prioritariamente das ruas de pulmão destacadas.
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          <div>
            ...
            <h3 className="text-sm font-black text-[#032b5e] uppercase tracking-wider mt-1">
              Lista Operacional de Quebras Estoque x Picking
            </h3>
          </div>
        ...
        {filteredItems.length === 0 ? (
          <div className="p-10 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-center flex flex-col items-center justify-center">
            ...
            <h4 className="text-sm font-black text-[#032b5e] uppercase tracking-wider">
              Nenhuma Quebra de Estoque x Picking Encontrada
            </h4>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-100/90 text-slate-700 border-b border-slate-200 uppercase text-[10px] font-black tracking-wider">
                  ...
                  <th className="py-3 px-4">Pulmão &amp; Validade Antiga</th>
                  <th className="py-3 px-4">Área de Picking &amp; Validade</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredItems.map((item, idx) => {
                  ...
                  return (
                    <tr key={`ep-row-${item.id}-${idx}`} className="hover:bg-slate-50/90 transition-colors">
                      ...
                      <td className="py-3.5 px-4 align-top bg-amber-50/20">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[9px] bg-amber-100 text-amber-800 font-black px-1 py-0.2 rounded">Pulmão</span>
                        </div>
                      ...
                      {/* Picking (Mais Novo) */}
                      <td className="py-3.5 px-4 align-top bg-sky-50/30">
                        <div className="flex items-center gap-1">
                          <span className="font-mono font-bold text-sky-800 text-xs">{item.validadeMaisNovo}</span>
                          <span className="text-[9px] bg-sky-100 text-sky-800 font-black px-1 py-0.2 rounded">Picking</span>
                        </div>
                        <span className="text-[10px] text-slate-700 font-black uppercase block mt-0.5">
                          📍 {item.posicaoDestino}
                        </span>
                      ...
                      <td className="py-3.5 px-4 align-top text-center">
                        {isDone ? (
                          <>
                            ...
                            <div className="text-[9px] text-slate-400">Abastecido no Prazo</div>
                          </>
                        ) : (
                          <>
                      ...
                      <td className="py-3.5 px-4 align-top text-center max-w-[220px]">
                        ...
                        <div className="mt-1.5 text-left text-[9.5px] text-slate-600 line-clamp-3 bg-slate-50 p-1.5 rounded border border-slate-200">
                          {item.tratativaDetalhada || item.observacao || 'Abastecimento imediato do picking executado com lote antigo.'}
                        </div>
                      </td>
          ...
          <div className="bg-white w-full max-w-xl rounded-2xl border border-slate-200 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <div className="bg-[#032b5e] text-white p-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase tracking-wider">Tratativa de Quebra Estoque x Picking</h3>
            ...
            <div className="p-5 flex flex-col gap-4 text-xs font-sans">
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 grid grid-cols-2 gap-2">
                <div>
                  ...
                  <span className="font-bold text-slate-800">Estoque x Picking (Abastecimento)</span>
                </div>
                <div>
                ...
                <div>
                  <span className="text-[10px] uppercase font-black text-slate-400 block">Pulmão Origem (Mais Antigo):</span>
                  <span className="font-bold text-rose-700">{selectedItemForEdit.posicaoOrigem}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-black text-slate-400 block">Destino Picking:</span>
              ...
              {/* Status Selector */}
              <div>
                <label className="block font-black text-slate-700 mb-1">Status da Tratativa / Abastecimento</label>
                <select
                  value={editStatus}
                ...
                >
                  <option value="concluido">✅ Concluído (Picking Abastecido com Lote Antigo com Sucesso)</option>
                  <option value="pendente">⏳ Pendente (Aguardando Retirada do Pulmão)</option>
                  <option value="em_andamento">🔄 Em Andamento (Em Remanejamento pelo Empilhador)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Descreva a ação corretiva tomada para abastecer o picking e eliminar a quebra..."
...
export default FefoEstoqueXPickingTab;
