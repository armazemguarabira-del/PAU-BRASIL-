...
export default function ConferenteView({
  ...
  const handleAddMapTag = (value: string) => {
    ...
    setSelectedRouteMaps(prev => {
      ...
      if (firstRoute && firstRoute.driverId) {
        setDriverId(firstRoute.driverId);
      }
  ...
  const handleStartNewSession = (e: React.FormEvent) => {
    ...
    const initialAssets: AuditAssetItem[] = activeAssets.map(asset => ({
      assetId: asset.id,
  ...
  const handleFinishPhysicalAudit = () => {
    if (!activeSession) return;
    ...
    if (refugoPhotos.length > 0 && updatedRefugos.length > 0) {
      updatedRefugos = updatedRefugos.map((ref, index) => {
        const photoUrl = refugoPhotos[index % refugoPhotos.length] || refugoPhotos[0];
        return { ...ref, photoUrl };
      });
      
      // Save all captured photos to the IndexedDB database
      refugoPhotos.forEach((photo, pIdx) => {
        savePhotoRecordToDb(
          photo,
  ...
  const savePhotoRecordToDb = async (base64Photo: string, itemCode: string, comment: string, photoType: 'produto' | 'ativo' | 'refugo' | 'troca_reposicao') => {
    if (!activeSession) return;
                          ...
                          <div className="flex items-center space-x-3">
                            {ref.photoUrl ? (
                              <div className="w-10 h-10 rounded border border-slate-200 overflow-hidden bg-slate-50 shrink-0">
                                <img src={ref.photoUrl} alt="Thumbnail" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                              </div>
                            ) : (
