  ...
  const handleFinalizeReconciliation = () => {
    ...
    const executeFinalization = async () => {
      ...
      try {
        ...
        // 3. CICLO DE VIDA DAS FOTOS: Mantidas no banco conforme regra de ouro e deletadas apenas pelo job assíncrono após 24h
        console.log("[Baixa Saga] Fotos de evidência preservadas em cache para auditoria segura. O job assíncrono do servidor efetuará a limpeza após 24h.");
        ...
        <div className="space-y-8">
          ...
          {activeTab === 'divergencias' && (
            <div className="space-y-6 animate-fade-in" id="tab_divergencias">
              ...
              {/* Filter controls */}
              <div className="bg-white p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4 shadow-3xs items-end">
                {/* Buscar */}
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">BUSCAR MAPA / MOTORISTA</span>
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Buscar NB, Mapa ou Placa..."
                      value={filterNB}
                ...
                <div className="h-9">
                  <button
                    ...
                    className="w-full h-full bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 text-xs font-bold uppercase rounded-lg transition cursor-pointer border border-slate-200"
                  >
                    Limpar Filtros
                  </button>
                </div>
